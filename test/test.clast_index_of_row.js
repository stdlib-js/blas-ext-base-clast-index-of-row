...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof clastIndexOfRow, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 9', function test( t ) {
	t.strictEqual( clastIndexOfRow.length, 9, 'returns expected value' );
	t.end();
});
...
tape( 'the function throws an error if provided a first argument which is not a valid order', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			clastIndexOfRow( value, data.M, data.N, new Complex64Array( data.A ), data.LDA, new Complex64Array( data.x ), data.strideX, new Uint8Array( data.M ), 1 );
		};
	}
});
...
tape( 'the function throws an error if provided a fifth argument which is not a valid `LDA` value (row-major)', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			clastIndexOfRow( data.order, data.M, data.N, new Complex64Array( data.A ), value, new Complex64Array( data.x ), data.strideX, new Uint8Array( data.M ), 1 );
		};
	}
});
...
tape( 'the function returns an invalid index when M is less than or equal to zero', function test( t ) {
	...
	data = ROW_MAJOR_DATA;
	out = clastIndexOfRow( data.order, 0, data.N, new Complex64Array( data.A ), data.LDA, new Complex64Array( data.x ), data.strideX, new Uint8Array( data.M ), 1 );
...
tape( 'the function returns an invalid index when N is less than or equal to zero', function test( t ) {
	...
	data = ROW_MAJOR_DATA;
	out = clastIndexOfRow( data.order, data.M, 0, new Complex64Array( data.A ), data.LDA, new Complex64Array( data.x ), data.strideX, new Uint8Array( data.M ), 1 );
...
tape( 'the function returns the index of the last row matching a search vector (row-major)', function test( t ) {
	...
	data = ROW_MAJOR_DATA;
	out = clastIndexOfRow( data.order, data.M, data.N, new Complex64Array( data.A ), data.LDA, new Complex64Array( data.x ), data.strideX, new Uint8Array( data.M ), 1 );
